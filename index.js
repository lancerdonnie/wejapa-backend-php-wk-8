...
blogs.addEventListener('click', async (e) => {
  ...
  if (e.target.classList.contains('fa-trash-o')) {
    ...
    const { data } = axios.post('/api/blogs/delete.php', { id });
    console.log(data);
    clearAndRefresh();
  } else {
    const id = e.target.closest('.card').id;
    // window.location = '/api/blogs/readone.php?id=' + id;
    window.location = '/blog.php?id=' + id;
  }
});
...
form.addEventListener('submit', async (e) => {
  ...
  const res = axios.post('/api/blogs/create.php', x);
  if ((res.status = '200')) {
    closeForm();
    clearAndRefresh();
  }
